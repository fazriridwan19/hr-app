...
import { AttendanceStatusBadge, AttendanceTypeBadge } from '@/components/ui/Badge';
...
export const AttendanceTable: React.FC<AttendanceTableProps> = ({
  data, loading, page, onPageChange, showEmployee = false,
}) => {
  const columns: Column<Attendance>[] = [
    ...(showEmployee ? [{
      ...
      render: (row: Attendance) => (
        <div>
          <p className="text-sm font-medium" style={{ color: 'var(--text-heading)' }}>{row.employeeName}</p>
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{row.employeeCode}</p>
        </div>
      ),
    ...
    },
    {
      ...
      header: 'Jam',
      render: (row) => (
        <span className="font-mono text-sm font-medium tabular" style={{ color: 'var(--text-heading)' }}>
